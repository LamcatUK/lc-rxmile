...
function catch_that_image( $post = null ) {
    ...
    if ( ! $post instanceof WP_Post ) {
        return esc_url( get_stylesheet_directory_uri() . '/img/blog-default.jpg' );
    }

    $first_img = '';

    if ( preg_match( '/<img[^>]+src=[\'"]([^\'"]+)[\'"]/i', $post->post_content, $matches ) ) {
        $first_img = trim( $matches[1] );
    }

    if ( ! $first_img || ( ! wp_http_validate_url( $first_img ) && 0 !== strpos( $first_img, '/' ) ) ) {
        $first_img = get_stylesheet_directory_uri() . '/img/blog-default.jpg';
    }
